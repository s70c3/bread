...
class Producer:
    ...
    def add_stream(self, request_id, rtsp, selection_area, counting_line, status):
        ...
        LINE_START = sv.Point(counting_line[0] - selection_area[1], counting_line[1] - selection_area[2])
        LINE_END = sv.Point(counting_line[2] - selection_area[1], counting_line[3] - selection_area[2])
        if status == 0:
            return
